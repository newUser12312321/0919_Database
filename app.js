...
res.render('notes/index', { title: '메모목록', notes });
});
app.get('/notes/new', (req, res) => {
res.render('notes/new', { title: '새메모' });
});
app.post('/notes', (req, res) => {
const { title, body } = req.body;
if (!title || !body) {
return res.status(400).render('notes/new', { title: '새메모', error: '제목과내용을입력하세요.'}) }
...
console.error(err);
res.status(500).render('500', { title: '서버오류', error: err });
});
app.listen(PORT, () => {
